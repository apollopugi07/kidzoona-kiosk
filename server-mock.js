...
let paymentResolver = null; // ADD THIS LINE HERE
...
app.post('/api/announce', async (req, res) => {
    ...
    console.log(`🔊 [MOCK AUDIO] Real-time Audio: "${safeText}"`);
    ...
    if (platform === 'win32') {
        ...
        const piperPath = "/home/kidzoona/piper/piper";
        const modelPath = "/home/kidzoona/piper/en_US-amy-medium.onnx";
        command = `echo "${safeText}" | ${piperPath} --model ${modelPath} --output_file - | aplay -q`;
    }

    exec(command, (error) => {
        if (error) console.log("[MOCK AUDIO] TTS Error (Ignored if Piper missing):", error);
    });
...
// MAIN REGISTRATION (MOCKED HARDWARE)
app.post('/api/register', async (req, res) => {
    try {
        ...
        console.log(`[MOCK HARDWARE] Would send Command to Arduino: ${command.trim()}`);
        console.log(`[MOCK HARDWARE] Simulating bill insertion for ₱${totalAmount}...`);

        // REVISED: Capture the result and assign the global resolver
        const result = await new Promise((resolve) => {
            paymentResolver = resolve; 

            // Save the interval to a global variable so the cancel route can stop it
            global.mockPaymentInterval = setInterval(() => {
                if (currentPaidAmount < totalAmount) {
                    currentPaidAmount += 100;
                    if (currentPaidAmount > totalAmount) {
                        currentPaidAmount = totalAmount; 
                    }
                    console.log(`[MOCK HARDWARE] Inserted bill! Paid: ₱${currentPaidAmount} / ₱${totalAmount}`);
                }
                
                if (currentPaidAmount >= totalAmount) {
                    clearInterval(global.mockPaymentInterval);
                    paymentResolver = null; // Clear it out
                    setTimeout(() => resolve({ cancelled: false }), 800); 
                }
            }, 1000); 
        });

        // REVISED: Check if the transaction was cancelled by the user or timeout
        if (result && result.cancelled) {
            console.log("[MOCK HARDWARE] ❌ Payment aborted. Registration discarded.");
            return res.status(400).json({ success: false, message: "Transaction Cancelled." });
        }

        console.log("[MOCK HARDWARE] 💰 Payment Confirmed!");
        ...
        if (req.body.guardianVoucherCode) {
            await Voucher.findOneAndDelete({ code: req.body.guardianVoucherCode.toUpperCase() });
        }
...
app.post('/api/cancel-payment', (req, res) => {
    console.log("🚫 [MOCK] Transaction cancelled by user or idle timeout.");

    // Stop the simulated bill insertion loop
    if (global.mockPaymentInterval) {
        clearInterval(global.mockPaymentInterval);
    }
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`🚀 [MOCK MODE] Server running on http://127.0.1.1:${PORT}`);
    console.log(`⚠️ Hardware requirements (SerialPort) have been bypassed for testing.`);
});